  ...
  Future<void> _handleReset() async {
    ...
    try {
      final res = await Dio().post(
        '${ApiConfig.nodeApi.replaceAll('/api', '')}/api/auth/forgot-password',
        data: {'email': _inputController.text.trim()},
      );
      if (res.data['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(res.data['message'] ?? 'Failed to send reset email.'),
    ...
    return Container(
      ...
      child: ElevatedButton(
        ...
        child: _isLoading
            ...
            : const Text("SEND RECOVERY CODE", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 1)),
      ),
    );
  }
}
